...
def _load_rules() -> dict[str, Any]:
    ...
    if not RULES_PATH.exists():
        raise RuntimeError(f"legit_rules_flat.json was not found at {RULES_PATH}")
...
def _root_key_from_serial(root_serial: Any) -> str:
    raw = str(root_serial or "").strip()
    if "|" in raw:
    ...
    try:
        ...
        wanted = -1
    if wanted < 0:
        raise ValueError("Choose a root variant first.")
    for root in roots():
        try:
            if int(root.get("serial") or -1) == wanted:
                return str(root.get("key") or "")
        except Exception:
            pass
    raise ValueError(f"No buildable root found for serial {wanted}.")


def _selected_part_lines(selected_parts: Any) -> list[str]:
    rows = selected_parts.splitlines() if isinstance(selected_parts, str) else list(selected_parts or [])
    out: list[str] = []
    for row in rows:
...
def validate_build(root_serial: Any, selected_parts: Any, unlock_modded: Any = False, level: Any = 60, signature: Any = 1) -> dict[str, Any]:
    ...
    validation = validate(root_key, selected)
    valid = bool(validation.get("ok"))
    ok = valid or unlocked
    errors = list(validation.get("errors") or [])
    if unlocked:
        status = "Unlocked: rules bypassed for modded gear, including duplicate parts. Verify output against save-editor.be."
    elif valid:
        status = f"Valid compact build: {validation.get('part_count', 0)} part(s)."
    else:
...
def build_human_external(root_serial: Any, selected_parts: Any, unlock_modded: Any = False, level: Any = 60, signature: Any = 1) -> dict[str, Any]:
    ...
    if not result["ok"]:
        return result
    human = build_human(result["root_key"], result["selected"], level=result["level"], seed=2, seed2=result["signature"])
...
def build_base85_external(root_serial: Any, selected_parts: Any, unlock_modded: Any = False, level: Any = 60, signature: Any = 1) -> dict[str, Any]:
    ...
    if not human:
        return result
    status = (
        "Built unlocked Base85 serial with rules/duplicate checks bypassed. Verify output against save-editor.be."
        if result.get("unlocked")
        else "Built valid Base85 serial."
    )
    return {**result, "base85": human_to_serial(human), "status": status}
